...
<html>
  <head>
	...
	var message=null;
	 ...
	 var bool=true;
	  function open(){
	  url="<%=path%>/note/addlibraryBook.action";
	  ...
	  if(bool){
	  	 ...
	  	 var bool=true;
	  	  var id=null ;
	  function update(){
	   url="<%=path%>/note/updatelibraryBook.action";
	   document.getElementById("fm").action=url;
	    if(id!=null){
	  var tableId = document.getElementById("booktable");
         $("#libraryName").val(tableId.rows[index].cells[1].innerHTML);
         $("#libraryId").val(tableId.rows[index].cells[2].innerHTML);
	  $("#dlg").css("display","none");
	  var aiin = document.getElementById("dlg");
	  if(bool){
	  		  aiin.style.display = 'block';
	  ...
	  function deletebook(){
	   ...
	   alert("宁确定要删除"+cartItemIdArray.length+"数据");
	       window.location.href="<%=path%>/note/delectlibraryBook.action?id="+cartItemIdArray;
	 ...
	 function firstpager(e){
	  ...
	  pagercount=$("#zuidayem").val();
	    window.location.href="<%=path%>/note/FirstlibraryBook.action?ee="+ee+"&pager="+pager+"&pagercount="+pagercount;
	 }
	  var mohuName;
	 function selectbook(){
	     mohuName= $("#selectname").val();
	      window.location.href="<%=path%>/note/libraryBook.action?mohulbtName="+encodeURIComponent(mohuName);
	 }
...
<table  align="left" width="680" border="0" cellspacing="2" style="position:relative"cellpadding="0">
  <tr>
    <td align="center" height="50px" bgcolor="#999999">图书馆类型    <font color="#FF0000">深色的为修改目标</font>    <a style="float: right; width: 200 ;font-size: 20" href="<%=path%>/mess/shouyeNews.html">首页</a> </td>
  </tr>
  <tr>
      ...
      <tr valign="top">
        <td><table id="booktable" width="1250" height="450" border="0" >
          <tr  height="2px"> <!-- bgcolor="B1BCEC" -->
            ...
            <td width="" align="center" valign="middle" bgcolor="#B1BCEC">图书馆名称</td>
             <td width="" align="center" valign="middle" bgcolor="#B1BCEC">图书馆编码</td>
            </tr>
            <c:forEach items="${lirb}" var="lib">
          <tr onclick="clicke(this.sectionRowIndex,${lib.libraryID })" onmouseover="checkaaaaa(${lib.libraryID })" height="10px">
          <td align="center"  valign="middle" > <input type="checkbox"  id="sele${lib.libraryID }"  name="checkboxBtn" onclick="xiugai(${lib.libraryID })" value="${lib.libraryID }" style="width: 20;height: 20"> </td>
            <td align="center"  valign="middle" >${ lib.libraryName}</td>
           <td align="center"  valign="middle" >${ lib.libraryID}</td>
            </tr>
                </c:forEach>
        </table>
        </td>
...
<div id="tb" style="position:absolute; float: left; margin:5px 10px 0 0">
	...
	<div>
		&nbsp;图书馆名称：&nbsp;<input type="text"   id="selectname"  size="20"/>
...
<div id="dlg" class="" style="position:absolute;margin:150px 100px 160px 500px ; background-color:yellow;
  ...
  <form id="fm"  method="post" >
  	<table cellspacing="8px;">
  	    ...
  	    <td style="width: 150px"><input class="" id="libraryName" name="lb.libraryName"  required="true" /></td>
  		<tr>
  		<tr>
  		   <td colspan="2">
  		...
  		<tr style="display: none">
  			<td >
  			<input class="" id="libraryId" name="lb.libraryId"  required="true"/>
  			</td>
  		</tr>
  	</table>
  </form>
